...
export default function CrystallineGrid() {
  ...
  useEffect(() => {
    ...
    const explosions: { x: number, z: number, time: number, maxTime: number, power: number }[] = [];
    const travelingParticles: { r: number, c: number, targetR: number, targetC: number, progress: number, speed: number }[] = [];
    ...
    const draw = () => {
      ...
      for (const tp of travelingParticles) {
